...
#' @param man_type Numeric: 0,1, or 2. 0= %BA removed from each classes during
#'   thinning; 1= Thinning is performed till %BA is removed (should be used in
#'   combination with thinning regime); 2= not used yet.
#' @param thinning_regime Character:"above" or "below".
...
#' @param rep_man: Character: management of replanted species (timber or
#'   coppice). c("T","C");
...
#' @param annual_man_out Character: Output management in a different output files? c("on","off");
...
#' @param cold_accl Character: Cold acclimation; options : c("on","off");
#' @param n_reg Character: Nitrongen limitation: c("on","off");
...
#' @param tll Numeric: tree height layer limit, to consider different trees in different layers
#' @param branch_rem Numeric: percentage of branch removed from the stand.
#'   Default to 100% (all branches removed)
#' @param inputdir Character: Path to input files directory
#' @param outputdir Character: Path to output files directory
#' @param check_meteo Logical: Should the validity of the input argument be checked?
...
run_3DCMCCFEM_5.7 <- function(site = NULL,
                          species = NULL,
                          year_start = NULL,
                          year_end = NULL,
                          co2 = "on",
  ...
  if (check_input){
    ...
    if (man == "on" | man == "var") {
      for (i in length(species):1) {
        sp <-
          read.table(text = stringr::str_squish(gsub("//.*", "", readLines(
            paste0(inputdir, "/", species, ".txt")[i]
          ))), sep = " ")
        ...
        if (man == "on") {
          ...
          if (sp[sp$V1 == "ROTATION", "value"] == -9999 ||
            ...
            stop(
              paste0(
                "\n'man' = on: \n* Missing management parameter(s) in '",
                species[i],
                ".txt'."
              )
            )
  ...
  settings <- rbind(data.frame("Parameters" = "SITENAME", "Values" = site),
                    ...
                    data.frame("Parameters" = "ANNUAL_MAN_OUTPUT", "Values" = annual_man_out),
                    ...
                    data.frame("Parameters" = "Cold_accl", "Values" = cold_accl),
                    data.frame("Parameters" = "N_REG", "Values" = n_reg),
                    data.frame("Parameters" = "regeneration", "Values" = reg),
                    data.frame("Parameters" = "management", "Values" = man),
                    data.frame("Parameters" = "management_type", "Values" = man_type),
                    ...
                    data.frame("Parameters" = "TREE_LAYER_LIMIT", "Values" = tll),
                    data.frame("Parameters" = "SOIL_LAYER", "Values" = 1),
                    data.frame("Parameters" = "MAX_LAYER_COVER", "Values" = 1.2),
                    data.frame("Parameters" = "THINNING_REGIME", "Values" =  thinning_regime),
                    data.frame("Parameters" = "BRANCH_REM", "Values" = branch_rem),
                    data.frame("Parameters" = "REPLANTED_SPECIES", "Values" = rep_species),
                    data.frame("Parameters" = "REPLANTED_MANAGEMENT", "Values" = rep_man),
  ...
  if(length(rep_species)>1){

    stopifnot("rep_man should be of the same length of rep_species"=length(rep_man)==length(rep_species),
              "rep_trees should be of the same length of rep_species"=length(rep_trees)==length(rep_species),
              "rep_age should be of the same length of rep_species"=length(rep_age)==length(rep_species),
              "rep_dbh should be of the same length of rep_species"=length(rep_dbh)==length(rep_species),
              "rep_height should be of the same length of rep_species"=length(rep_height)==length(rep_species))

    n <- seq_along(rep_species)-1
    ...
    for (i in n) {
    rep_list[[i+1]] <- rbind(data.frame("Parameters" = "REPLANTED_SPECIES", "Values" = rep_species[i+1]),
    data.frame("Parameters" = "REPLANTED_MANAGEMENT", "Values" = "T"),
    data.frame("Parameters" = "REPLANTED_TREE", "Values" = rep_trees[i+1]),
    data.frame("Parameters" = "REPLANTED_AGE", "Values" = rep_age[i+1]),
    data.frame("Parameters" = "REPLANTED_AVDBH", "Values" = rep_dbh[i+1]),
    data.frame("Parameters" = "REPLANTED_LAI", "Values" = rep_lai[i+1]),
    data.frame("Parameters" = "REPLANTED_HEIGHT", "Values" = rep_height[i+1]),
    data.frame("Parameters" = "REPLANTED_WS", "Values" = 0),
    data.frame("Parameters" = "REPLANTED_WCR", "Values" = 0),
    data.frame("Parameters" = "REPLANTED_WFR", "Values" = 0),
    data.frame("Parameters" = "REPLANTED_WL", "Values" = 0),
    ...
    rep_list[[i+1]][,1] <- paste0(rep_list[[i+1]][,1],"_",i)

    }

    rep_df <- do.call(rbind,rep_list)
    other <- rbind(data.frame("Parameters" = "REGENERATION_SPECIES", "Values" = species),
                   data.frame("Parameters" = "REGENERATION_MANAGEMENT", "Values" = "T"),
                   data.frame("Parameters" = "REGENERATION_N_TREE", "Values" = reg_trees),
                   data.frame("Parameters" = "REGENERATION_AGE", "Values" = reg_age),
                   data.frame("Parameters" = "REGENERATION_AVDBH", "Values" = reg_dbh),
    ...
    settings <- rbind(settings,rep_df,other)
  ...
  if(man == "off" || man == "on" || (man == "var" && pres_dens == "on" && after_pres == "off" && reg == "off")) {
    systemCall  <- paste0('\"', modeldir, '/3D_CMCC_Forest_Model_5.7\" ',
                          ' -i \"', inputdir, '/" ',
                          ' -p \"', inputdir, '/\"',
                          ' -o \"output/\"',
                          ' -d \"', site, '_stand.txt\"',
  ...
  if(move_and_rename){
    pattern <- paste0(site,"_co2_",co2,"_man_",man,"_",year_start,"-",year_end,"_",output,".txt")
    #rename debug file
    debug <- list.files(outputdir,recursive = F,full.names = T)
    tmpout <- debug[grepl("output_5.6",basename(debug),fixed=T)]
    debug <- debug[grep(pattern,basename(debug))]
    ...
    if(output == "annual") {
      ...
      outfile <- sort(list.files(outfold,full.names = T,pattern = "txt"))
      if(man == "off" || man == "on") {
        file.rename(from = outfile,
                    to = file.path(outputdir, paste0(sub("\\d.*", "", basename(outfile)),site, "_co2_", co2, "_man_", man, "_", year_start, "-", year_end, ".txt")))
      }
      if(man == "var") {
        file.rename(from = outfile,
                    to = file.path(outputdir, paste0(sub("\\d.*", "", basename(outfile)),site, "_co2_", co2, "_man_", man, "_pres_dens_", pres_dens, "_reg_", reg, "_", year_start, "-", year_end, ".txt")))
      }
    }
    if(output == "monthly") {
