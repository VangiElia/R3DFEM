...
make_CCS <- function(meteo=NULL,co2=NULL,obs_year=NULL,end_year=2099,outdir=NULL){
...
# args check --------------------------------------------------------------
  ...
  if(class(meteo)=="character"){
    ...
    meteo <- meteo
  }else stop("meteo is of the wrong class")
  ...
  if(class(co2)=="character"){
    ...
    co2 <- co2
  }else stop("co2 is of the wrong class")
...
# - -----------------------------------------------------------------------
  ...
  anni_random <-
    sample(obs_year[1]:obs_year[2], end_year - obs_year[2], replace = T)
...
# internal data loading ---------------------------------------------------
  ...
  meteo_random <- do.call(rbind,lapply(seq_along(anni_random$anni_or),function(i){
 ...
 co2_random <- do.call(rbind,lapply(seq_along(anni_random$anni_or),function(i){
